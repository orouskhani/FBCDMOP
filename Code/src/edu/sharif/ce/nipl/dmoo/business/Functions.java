...
public class Functions {
	...
	public List<Integer> rankAggeragationAlgorithm(List<Cat> population1 , List<Cat> population2) {
		...
		for (int i = 0 ; i < population1.size() ; i++) {
			double scoreInPop1 = 0;
			for (Cat cat : population1) {
				if(cat.getId() == i){
					scoreInPop1 = cat.getRank() + 1;
				}
			}
			double scoreInPop2 = 0;
			for (Cat cat : population2) {
				if(cat.getId() == i){
					scoreInPop2 = cat.getRank() + 1;
				}
			}
			 //Modified Borda Start
			double finalScore1 = (1 + Math.floor(scoreInPop1 / 10) / 10) * scoreInPop1;
			double finalScore2 = (1 + Math.floor(scoreInPop2 / 10) / 10) * scoreInPop2;
			double stabilityScore = (Math.abs(scoreInPop1 - scoreInPop2))/(Math.abs(scoreInPop1 + scoreInPop2));
			finalScore.add(0.2 * (finalScore1) + 0.8 * (finalScore2) + stabilityScore );// Modified Borda End
