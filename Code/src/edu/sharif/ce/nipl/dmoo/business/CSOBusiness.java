...
public class CSOBusiness {
	...
	private double lastHVRValue;

	public CSOBusiness(int nPop , int nVar1 , int nVar2 , double minVar1 , double maxVar1 , double minVar2 , double maxVar2 , int func , int cycle , int towet , int nt, int selectionAlgo, int reinitAlgo , int max_iter){
	...
	public void CSOMovement(int iter) throws IOException{
		...
		// TODO Comment
		//MR = Coeffs.calMR(default_MR , last_change , iter , max_iter); // must be 0.98
		//SMP = Coeffs.calSMP(last_change , iter , max_iter);
		//SRD = Coeffs.calSRD(last_change , iter , max_iter);
		...
		resultOfTracingMode = CSOTracingModeForCats(tracingCats , iter);		
	...
	private ArrayList<Cat> CSOTracingModeForCats(ArrayList<Cat> tracingCats , int iter) {
		// in evaluation we must have best cat for each function.
		ArrayList<Cat> result = new ArrayList<Cat>();
		
		// TODO Comment
		//C = Coeffs.calC(last_change , iter , max_iter);
		//W = Coeffs.calW(last_change , iter , max_iter);
	...
	//Calculating the change value
	public void setCDC(int iter) {
		List<Cat> afterChangeCats = new ArrayList<Cat>();
		for(int i = 0 ; i < population.size() ; i++){
			afterChangeCats.add((Cat)population.get(i).clone());
		}
		//calculate the change for first function
		double averageFunction1BeforeChange = findAverageFitness(population , 0, iter - 1);
		double averageFunction1AfterChange = findAverageFitness(afterChangeCats, 0, iter);
		double NormalchangeValue1 = Math.abs((double)(averageFunction1AfterChange - averageFunction1BeforeChange)/(averageFunction1AfterChange + averageFunction1BeforeChange));
		
		//calculate the change for second function
		double averageFunction2BeforeChange = findAverageFitness(population, 1, iter - 1);
		double averageFunction2AfterChange = findAverageFitness(afterChangeCats, 1, iter);
		double NormalchangeValue2 = Math.abs((double)(averageFunction2AfterChange - averageFunction2BeforeChange)/(averageFunction2AfterChange + averageFunction2BeforeChange));
		
		double AverageNormalchangeValue = (NormalchangeValue1 + NormalchangeValue2) / 2;
		
		System.out.println("Old CDC: " + CDC);
		System.out.println("HVR: " + lastHVRValue);
		System.out.println("Average Normal Change Value " + AverageNormalchangeValue);
		
		CDC = Coeffs.calCDC(CDC, lastHVRValue, AverageNormalchangeValue);
		System.out.println("New CDC: " + CDC);
		
	}
	
	//Calculating the average fitness of a population
	private double findAverageFitness(List<Cat> population2, int numFunc, int iter) {
		double sum = 0;
		for(int i = 0 ; i < population2.size() ; i++){
			sum += benchmark.evaluate(population2.get(i), numFunc , iter);
		}
		return sum / population2.size();
	}
